...
public:
    ...
    // Ïåðåîïðåäåëåíèå ôóíêöèè determinant äëÿ âîçâðàòà ñóììû ýëåìåíòîâ íà ïîáî÷íîé äèàãîíàëè
...
int main() {
    ...
    matrixPtr = &matrix1;
    cout << "Îïðåäåëèòåëü áàçîâîé ìàòðèöû: " << matrixPtr->determinant() << '\n';

    matrixPtr = &derivedMatrix;
    cout << "Îïðåäåëèòåëü ïðîèçâîäíîé ìàòðèöû (ñóììà äèàãîíàëåé): " << matrixPtr->determinant() << '\n';
